import * as h from './herramientas'
import * as t from './Tarea';
import { verDetalles, mostrarCoincidencias } from './menus';
export const listaTareas: t.Tarea[]=[];
...
export function verConCondicion(s: string){
    let band:number=0;
    const arrayIndices: number[]= [];
...
export function mostrarTarea(i:number){
    console.log("-----------------------------------");
...
export function editarTarea(i:number){
    let editar: String, fecha:string, tareaAux: t.Tarea;
...
export function ordenarTareas(){
    let tareaAux : t.Tarea;
    for(let i:number=0;i<listaTareas.length;i++){
        for(let j:number=0;j<listaTareas.length-1;j++){
...
export function buscarTarea(cadena:string){
    let arrayIndices: number []=[], band: number=0;
    for(let i=0;i<listaTareas.length;i++){
